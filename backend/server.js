...
app.post("/api/tax", (req, res) => {
  // Handle saving tax data to the database or any other processing
  // Logic for saving tax data goes here
...
// search api using query params

app.get("/api/search", (req, res) => {
  // Return data after search from the data object using query params
  ...
  const results = data.filter((item) => {
    ...
    return item.name.includes(name); // Include a return statement here
  });
...
app.get('/api/items', (req, res) => {
    // get items using category from query
    const { category } = req.query;
    if (!category) {
      // Return all items if no category is provided
      const results = data.filter((item) => !item.category || item.category.name === category);
    ...
    const results = data.filter((item) => {
      if (!item.category) {
        return true; // Include items with no category
      }
